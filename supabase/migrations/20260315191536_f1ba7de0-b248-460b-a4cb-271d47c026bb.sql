...
SELECT
  ...
  (SELECT count(*) FROM public.prompts p INNER JOIN public.profiles pr ON p.user_id = pr.id WHERE pr.is_admin = false AND p.preview_url IS NOT NULL)::bigint AS total_renders,
  (SELECT count(*) FROM public.prompts p INNER JOIN public.profiles pr ON p.user_id = pr.id WHERE pr.is_admin = false AND p.preview_url IS NOT NULL AND p.created_at >= now() - interval '30 days')::bigint AS renders_30d,
